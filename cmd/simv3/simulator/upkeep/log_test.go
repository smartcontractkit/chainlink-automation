...
func TestLogTriggerTracker(t *testing.T) {
	...
	broadcaster := chain.NewBlockBroadcaster(conf, 1, logger, loadUpkeepAt(upkeep1, 3), loadLogAt(chainLog1, 4))
...
func TestLogTriggerTracker_NoUpkeeps(t *testing.T) {
	...
	broadcaster := chain.NewBlockBroadcaster(conf, 1, logger, loadLogAt(chainLog1, 4))
	listener := chain.NewListener(broadcaster, logger)
	active := upkeep.NewActiveTracker(listener, logger)
	performs := upkeep.NewPerformTracker(listener, logger)
...
func TestLogTriggerTracker_Performed(t *testing.T) {
	...
	broadcaster := chain.NewBlockBroadcaster(conf, 1, logger, loadUpkeepAt(upkeep1, 3), loadLogAt(chainLog1, 4), loadPerformAt(perform1, 5))
	listener := chain.NewListener(broadcaster, logger)
	active := upkeep.NewActiveTracker(listener, logger)
	performs := upkeep.NewPerformTracker(listener, logger)
