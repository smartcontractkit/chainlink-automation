...
func (keyring *OffchainKeyring) OffchainSign(msg []byte) (signature []byte, err error) {
	return ed25519.Sign(ed25519.PrivateKey(keyring.signingKey), msg), nil
}
...
func (keyring *OffchainKeyring) ConfigDiffieHellman(point [curve25519.PointSize]byte) ([curve25519.PointSize]byte, error) {
	p, err := curve25519.X25519(keyring.encryptionKey[:], point[:])
	if err != nil {
...
func (keyring *OffchainKeyring) OffchainPublicKey() types.OffchainPublicKey {
	var offchainPubKey [ed25519.PublicKeySize]byte
	copy(offchainPubKey[:], keyring.signingKey.Public().(ed25519.PublicKey)[:])
	return offchainPubKey
}

// ConfigEncryptionPublicKey returns config public key
func (keyring *OffchainKeyring) ConfigEncryptionPublicKey() types.ConfigEncryptionPublicKey {
	cpk, _ := keyring.configEncryptionPublicKey()
	return cpk
}

func (keyring *OffchainKeyring) configEncryptionPublicKey() (types.ConfigEncryptionPublicKey, error) {
	rv, err := curve25519.X25519(keyring.encryptionKey[:], curve25519.Basepoint)
	if err != nil {
		return [curve25519.PointSize]byte{}, err
	}
...
func (keyring *EvmKeyring) PublicKey() types.OnchainPublicKey {
	return keyring.signingAddress().Bytes()
}

// XXX: PublicKey returns the address of the public key not the public key itself
func (keyring *EvmKeyring) PKString() string {
	return keyring.signingAddress().String()
}

func (keyring *EvmKeyring) reportToSigData(digest types.ConfigDigest, v uint64, r ocr3types.ReportWithInfo[plugin.AutomationReportInfo]) []byte {
	rawRepctx := [3][32]byte{}
...
func (keyring *EvmKeyring) Sign(digest types.ConfigDigest, v uint64, r ocr3types.ReportWithInfo[plugin.AutomationReportInfo]) ([]byte, error) {
	return crypto.Sign(keyring.reportToSigData(digest, v, r), &keyring.privateKey)
}

func (keyring *EvmKeyring) Verify(publicKey types.OnchainPublicKey, digest types.ConfigDigest, v uint64, r ocr3types.ReportWithInfo[plugin.AutomationReportInfo], signature []byte) bool {
	hash := keyring.reportToSigData(digest, v, r)
...
func (keyring *EvmKeyring) MaxSignatureLength() int {
	return 65
}

func (keyring *EvmKeyring) signingAddress() common.Address {
	return crypto.PubkeyToAddress(*(&keyring.privateKey).Public().(*ecdsa.PublicKey))
}

func (keyring *EvmKeyring) Marshal() ([]byte, error) {
	return crypto.FromECDSA(&keyring.privateKey), nil
}

func (keyring *EvmKeyring) Unmarshal(in []byte) error {
