...
- Every second (via log recovery ticker), query for a random subset of upkeeps and a random range of blocks (above the block number from life cycle manager) within a limit
- Put logs into recovery flow without checkBlockNum, checkBlockHash

<aside>
💡 Note: This is a sampling approach which divides the work randomly across nodes. There are alternative approaches where a node does all the work on its own or maintains consistent block ranges for better guarantees

</aside>

<aside>
💡 Note: When querying log poller from DB for random subset of logs, they can be filtered within the DB query itself, the logs which do not have any associated result. Essentially we query DB for ‘un-finalised logs’.
</aside>

<aside>
💡 Alternates can include a DHT approach to divide upkeeps among nodes along with some redundancy for it to account for malicious nodes. For an upkeep the node maintains a pointer to a block number till which logs have completely processed and attempts to increase this pointer on regular intervals
</aside>
