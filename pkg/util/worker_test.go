...
func TestWorker(t *testing.T) {
	t.Run("Return Complete Result to Result Function", func(t *testing.T) {
		w := &worker{
			Name:  "worker",
			Queue: make(chan *worker), // leave this unbuffered to not allow anything to block the queue
		}

		var c sync.Mutex
		var resultItem *WorkItemResult

		res := []ocr2keepers.UpkeepResult{"1"}

		f := func(_ context.Context) ([]ocr2keepers.UpkeepResult, error) {
			return res, fmt.Errorf("error")
		}

		ctx := context.Background()
		w.Do(ctx, func(item WorkItemResult) { c.Lock(); defer c.Unlock(); resultItem = &item }, f)
	...
	t.Run("Return Context Error on Cancelled Context", func(t *testing.T) {
		w := &worker{
			Name:  "worker",
			Queue: make(chan *worker), // leave this unbuffered to block the queue
		}
...
func TestWorkerGroup(t *testing.T) {
	...
	t.Run("All Work Done", func(t *testing.T) {
		wg := NewWorkerGroup(8, 1000)
		ctx := context.Background()
		group := 0

		closed := make(chan struct{})
		var done int
		go func(w *WorkerGroup, c context.Context) {
			for {
		...
		for n := 0; n < 10; n++ {
			err := wg.Do(ctx, func(c context.Context) ([]ocr2keepers.UpkeepResult, error) {
				return []ocr2keepers.UpkeepResult{"1"}, nil
			}, group)
			if err != nil {
				t.FailNow()
			}
	...
	t.Run("Error on Cancel and Full Queue", func(t *testing.T) {
		...
		wg := &WorkerGroup{
			svcCtx:           svcCtx,
			svcCancel:        svcCancel,
			input:            make(chan GroupedItem), // unbuffered to block
		...
		go func(errs chan error, gp int) {
			err := wg.Do(ctx, func(_ context.Context) ([]ocr2keepers.UpkeepResult, error) {
				return []ocr2keepers.UpkeepResult{"1"}, nil
			}, gp)
	...
	t.Run("Error on Stop and Full Queue", func(t *testing.T) {
		...
		go func(errs chan error, gp int) {
			err := wg.Do(context.Background(), func(_ context.Context) ([]ocr2keepers.UpkeepResult, error) {
				return []ocr2keepers.UpkeepResult{"1"}, nil
			}, gp)
	...
	t.Run("Stop Closes Queue and Ends Run Context", func(t *testing.T) {
		wg := NewWorkerGroup(1, 1)
		group := 0

		go func(gp int) {
			err := wg.Do(context.Background(), func(ctx context.Context) ([]ocr2keepers.UpkeepResult, error) {
				tmr := time.NewTimer(100 * time.Millisecond)
				select {
				case <-ctx.Done():
					tmr.Stop()
					return []ocr2keepers.UpkeepResult{"0"}, fmt.Errorf("error")
				case <-tmr.C:
					return []ocr2keepers.UpkeepResult{"1"}, nil
				}
			}, gp)
		...
		case <-wg.NotifyResult(group):
			for _, r := range wg.Results(group) {
				...
				assert.Equal(t, []ocr2keepers.UpkeepResult{"0"}, r.Data, "data from work result should match")
		...
		testAdd := func() error {
			return wg.Do(context.Background(), func(ctx context.Context) ([]ocr2keepers.UpkeepResult, error) {
				return []ocr2keepers.UpkeepResult{"0"}, nil
			}, group)
		}
...
func TestRunJobs(t *testing.T) {
	...
	//		wg := NewWorkerGroup(10, 100)
	//
	//		jobCount := 100
	//		jobs := make([][]ocr2keepers.UpkeepKey, jobCount)
	//		for i := 0; i < jobCount; i++ {
	//			jobs[i] = []ocr2keepers.UpkeepKey{[]byte("1")}
	//		}
	//
	//		var result int
	//		var errors int
