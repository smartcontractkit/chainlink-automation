...
func TestAutomationObservation_Encode(t *testing.T) {
	...
	expectedJSON := `{"Instructions":["instruction1","instruction2"],"Metadata":{"key":"value"},"Performable":[{"Payload":{"ID":"abc"},"Retryable":false}]}`

	data, err := observation.EncodeAutomationObservation()
...
func TestAutomationOutcome_Encode(t *testing.T) {
	...
	data, err := outcome.EncodeAutomationOutcome()
	assert.NoError(t, err)
