...
func TestUpkeepObservation_UnmarshalJSON(t *testing.T) {
	t.Run("valid bytes unmarshal successfully", func(t *testing.T) {
		var observation UpkeepObservation
		err := json.Unmarshal([]byte(`{"1":"123","2":["NDU2"]}`), &observation)
		assert.Nil(t, err)
		assert.Equal(t, UpkeepObservation{
			BlockKey: "123",
			UpkeepIdentifiers: []types.UpkeepIdentifier{
				types.UpkeepIdentifier("456"),
			},
		}, observation)
	})
